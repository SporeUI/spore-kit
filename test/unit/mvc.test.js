...
const $events = require('../../packages/evt/events');
...
describe('mvc.proxy', () => {
  ...
  test('proxy 方法用于绑定方法的执行环境', () => {
    ...
    const update = $proxy(obj, 'update');
    update(2);
    expect(obj.count).toBe(3);
  });
});

describe('mvc.delegate', () => {
  const $proxy = $mvc.proxy;
...
describe('mvc.klass', () => {
  ...
  test('实现静态方法', () => {
    ...
    const obj = new Cls();
    expect(obj.updateA).toBeUndefined();
    expect(typeof Cls.updateA).toBe('function');
...
describe('mvc.view', () => {
  ...
  test('设置 node 选项为元素 id 或者元素本身，可设置视图的根节点。', () => {
    ...
    const obj2 = new TestView();
    const obj3 = new TestView({
      node: node.get(0),
    });

    expect(obj1.role('root').attr('id')).toBe(id);
    expect(obj2.role('root').attr('id')).toBe(id);
    expect(obj3.role('root').attr('id')).toBe(id);

    node.remove();
  });

  test('设置 template 选项为 html 字符串，或者字符串数组，组件初始化后，可直接创建视图 dom。', () => {
    const id1 = $getUniqueKey();
    const id2 = $getUniqueKey();
    ...
    expect(obj1.role('root').attr('id')).toBe(id1);
    expect(obj2.role('root').attr('id')).toBe(id2);
  });

  test('同时设置 template 和 node ，优先使用 node 识别根元素。', () => {
    const id1 = $getUniqueKey();
    const id2 = $getUniqueKey();
    ...
    expect(obj.role('root').attr('id')).toBe(id1);

    node.remove();
  });

  test('使用 role 方法可获取根元素内部标记了 role 属性的元素。', () => {
    const id = $getUniqueKey();
    const obj = new $view({
      template: [
        '<div">',
    ...
    expect(obj.role('member').attr('id')).toBe(id);
  });

  test('可通过 role 选项指定选择器与对应的角色名称。', () => {
    const id = $getUniqueKey();
  ...
  test('可以用 resetRoles 方法清除视图缓存的角色 dom 元素。', () => {
    const id1 = $getUniqueKey();
    const id2 = $getUniqueKey();
    const obj = new $view({
      template: [
    ...
    expect(obj.role('member').attr('id')).toBe(id1);

    obj.resetRoles();
    obj.role('root').html(`<div role="member" id="${id2}"></div>`);

    expect(Object.keys(obj.nodes).length).toBe(1);
    expect(obj.role('member').attr('id')).toBe(id2);
    expect(Object.keys(obj.nodes).length).toBe(2);
  });

  test('在 events 对象上可以绑定代理事件', () => {
    const TPL = {
      box: [
        '<div>',
    ...
    expect(obj.arr.join('')).toBe('35');

    obj.role('root').remove();
  });

  test('在 events 上对一个选择器可以绑定多个代理事件', () => {
    const TPL = {
      box: [
        '<div>',
        '<button data-val="3"></button>',
    ...
    const TestView = $view.extend({
      ...
      add(evt) {
        const node = $(evt.currentTarget);
        this.val += parseInt(node.attr('data-val'), 10);
      },
    });
  ...
  test('配置 events 选项可自定事件代理。', () => {
    ...
    expect(obj1.val).toBe(1);
    expect(obj2.val).toBe(2);

    obj1.role('root').remove();
    obj2.role('root').remove();
  });

  test('view 组件提供 destroy 方法清除缓存对象。', () => {
    const TPL = {
      box: [
        '<div>',
        '<button role="button"></button>',
    ...
    expect(Object.keys(obj.nodes).length).toBe(0);
